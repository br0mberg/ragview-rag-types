...
Публичный пакет позволяет проверить хэши, позиции и расчёт метрик. Исходные
тексты корпуса и FAQ не опубликованы: для этого нет явного разрешения. Источники
описаны в [`DATA_SOURCES.md`](DATA_SOURCES.md).
...
явного разрешения на его публикацию. Сборщик получает данные с сайта ФНС и
сохраняет их локально вместе со ссылками на источники.

В релиз также не входят полный текст НК РФ, пакеты разметки, веса моделей и
локальные результаты. Они хранятся в `data/local/` и `results/local/`.
Отдельный скрипт проверяет, что эти файлы не попали в Git.

Происхождение данных и границы лицензии описаны в
[`DATA_SOURCES.md`](DATA_SOURCES.md). Репозиторий не является правовым
справочником, а техническая разметка по ссылкам не оценивает полноту ответа.
...
Автор — Андрей Бромбин, [Telegram](https://t.me/devbrombin). Ссылка на статью
появится здесь после публикации.

Авторский код распространяется по [MIT License](LICENSE). Границы лицензии для
данных, весов моделей и иллюстраций описаны в
